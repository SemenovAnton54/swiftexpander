...
class SwiftExpanderToSymbolsTests: XCTestCase {
    ...
    func testExample() throws {
        var value = expander.expandTo(text: "(foo - bar)", start: 7, end: 10)
        ...
        value = expander.expandTo(text: "(foo - bar)", start: 1, end: 10)
        ...
        value = expander.expandTo(text: "   ", start: 1, end: 2)
        XCTAssertNil(value)

        value = expander.expandTo(text: "(   ", start: 2, end: 2)
        XCTAssertNil(value)

        value = expander.expandTo(text: "console.log(foo.indexOf('bar') > -1);", start: 32, end: 32)
        XCTAssertNotNil(value)
        XCTAssertEqual(value?.start, 12)
        XCTAssertEqual(value?.end, 35)

        value = expander.expandTo(text: "(a['value'])", start: 6, end: 11)
        XCTAssertNotNil(value)
...
""", start: 10, end: 61)
        ...
        value = expander.expandTo(text: "{'a(a'+bb+'c)c'}", start: 8, end: 8)
        XCTAssertNotNil(value)
        XCTAssertEqual(value?.start, 1)
